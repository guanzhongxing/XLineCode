				...
				<c:forEach var="topic" items="${requestScope.userSpecifiedTopics}">
					<tr>
						<td class="row1" valign="middle" align="center" width="20"><img
							src="${resourcesHost}/images/folder_new_big.gif" alt="" /></td>
						<td class="row1" width="100%"><span class="topictitle">
								<a href="${contextPath}/forums/topics/${topic.id}">${topic.subject}</a>
						</span></td>
						...
						<td class="row3right" valign="middle" nowrap="nowrap"
							align="center"><c:set var="latestResponse"
								value="${statistician.latestResponse}" /> <span
							class="postdetails"> <c:choose>
									<c:when test="${not empty latestResponse}">
										...
										<fmt:formatDate value="${latestResponse.createdTime}"
											pattern="hh:mm" />
										<br />
										<a href="${contextPath}/users/${latestResponse.author.id}">${latestResponse.author.name}</a>
										<a
											href="${contextPath}/forums/topics/${topic.id}#${latestResponse.id}">
											<img src="${resourcesHost}/images/icon_latest_reply.gif"
						...
						<td class="row2" valign="middle" align="center"><a
							class="postdetails"
							href="${contextPath}/forums/${topic.forum.id}">${topic.forum.name}</a></td>
