						...
						<tr>
							<td valign="top" align="center" width="100%">
								<table cellspacing="2" cellpadding="2" border="0">
									<tbody>
										<tr valign="middle">
											<td><c:if test="${boxType!='inbox'}">
													<a href="${contextPath}/pms">
												</c:if> <img src="${resourcesHost}/images/msg_inbox.gif" border="0"
												alt="[Inbox]" /> <c:if test="${boxType!='inbox'}">
													</a>
												</c:if></td>
											<td><c:if test="${boxType!='inbox'}">
													<a href="${contextPath}/pms">
												</c:if> <span class="cattitle"><fmt:message
														key="pm_list.jsp.inbox" /> &nbsp;</span> <c:if
													test="${boxType!='inbox'}">
													</a>
												</c:if></td>
											<td><c:if test="${boxType!='sentBox'}">
													<a href="${contextPath}/pms?boxType=sentBox">
												</c:if> <img src="${resourcesHost}/images/msg_sentbox.gif"
												border="0" alt="[Sentbox]" /> <c:if
													test="${boxType!='sentBox'}">
													</a>
												</c:if></td>
											<td><c:if test="${boxType!='sentBox'}">
													<a href="${contextPath}/pms?boxType=sentBox">
												</c:if> <span class="cattitle"><fmt:message
														key="pm_list.jsp.sentBox" />&nbsp;</span> <c:if
													test="${boxType!='sentBox'}">
													</a>
												</c:if></td>
										</tr>
				...
				<form action="" method="post" name="privmsg_list" id="privmsg_list">
						...
						<tbody>
							<tr>
								<td valign="middle" align="left"><a
									href="${contextPath}/pms/form" class="icon_new_topic"><img
						...
						<tbody>
							<tr>
								...
								<th class="thtop" nowrap="nowrap" width="55%">&nbsp;<fmt:message
										key="all.jsp.msgSubject" />&nbsp;
								</th>
								<th class="thtop" nowrap="nowrap" width="20%"><c:choose>
								...
								<th class="thtop" nowrap="nowrap" width="15%">&nbsp;<fmt:message
										key="pm.jsp.date" />&nbsp;
								</th>
							...
							<c:forEach var="pm" items="${requestScope.privateMsgs}">
								<tr>
									...
									<td class="row1" valign="middle" width="55%"><span
										class="topictitle">&nbsp;<a class="topictitle"
											href="${contextPath}/pms/${pm.id}">${pm.subject}</a> <c:if
												test="${pm.newToReceiver}">
									...
									<td class="row1" valign="middle" align="center" width="20%">
										<c:choose>
											<c:when test="${boxType=='inbox'}">
												<span class="name">&nbsp;<a class="name"
													href="${contextPath}/users/${pm.author.id}">${pm.author.name}</a></span>
											</c:when>
											<c:otherwise>
												<span class="name">&nbsp;<a class="name"
													href="${contextPath}/users/${pm.receiver.id}">${pm.receiver.name}</a></span>
											</c:otherwise>
										</c:choose>
									</td>
									<td class="row1" valign="middle" align="center" width="15%">
										<span class="postdetails"> <fmt:formatDate
												value="${pm.createdTime}" dateStyle="medium" /> <fmt:formatDate
												value="${pm.createdTime}" pattern="hh:mm" />
									</span>
									</td>
									<td class="row1" valign="middle" align="center" width="5%"><span
										class="postdetails"><input type="checkbox"
											value="${pm.id}" name="id" /></span></td>
								</tr>
							</c:forEach>
							<tr>
								<td class="catbottom" align="right" colspan="5" height="28">
									&nbsp; <input class="liteoption" type="submit"
									value="<fmt:message key="pm_list.jsp.deleteButton"/>"
									name="delete" />
								</td>
							</tr>
						</tbody>
					...
					<table cellspacing="2" cellpadding="2" width="100%" align="center"
						...
						<tbody>
							<tr>
								<td valign="middle" align="left"><a
									href="${contextPath}/pms/form" class="icon_new_topic nav"><img
										src="${resourcesHost}/images/${locale}/post.gif" alt="" /></a></td>
								<td valign="middle" align="left" width="100%"><span
									class="nav"></span></td>
